...
class Square:
    ...
    def draw_it(self):
        pygame.draw.rect(screen, self.color,
                         (self.x, self.y, self.width, self.width), 0)
...
def apple_xy():
    apple.x = random.randint(
        0, (w / (grid.size + grid.distance))-1) * (grid.size + grid.distance)
    apple.y = random.randint(
        0, (h / (grid.size + grid.distance))-1) * (grid.size + grid.distance)
